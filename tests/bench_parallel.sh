...
ITERATION=$1
RESULTS_DIR="/tmp/grit-parallel-bench/iter-$ITERATION"
mkdir -p "$RESULTS_DIR"
...
TMP_IDX=/tmp/grit-sym-idx-$$-$ITERATION
rm -rf "$TMP_IDX"
cp -r "$PROJ" "$TMP_IDX"
...
for NUM_AGENTS in "${AGENT_COUNTS[@]}"; do
    ...
    for ROUND in $(seq 1 $NUM_ROUNDS); do
        ...
        for i in $(seq 1 $NUM_AGENTS); do
            ...
            (
                "$GRIT" --repo "$WORK" claim -a "i${ITERATION}n${NUM_AGENTS}r${ROUND}a${i}" -i "task$i" "${AGENT_SYMS[@]}" >/dev/null 2>&1 || exit 1
                WT="$WORK/.grit/worktrees/i${ITERATION}n${NUM_AGENTS}r${ROUND}a${i}"
                if [[ -d "$WT" ]]; then
                    for SYM in "${AGENT_SYMS[@]}"; do
                        modify_function "${SYM%%::*}" "${SYM##*::}" "a$i-r$ROUND" "$WT"
                    done
                fi
                "$GRIT" --repo "$WORK" done -a "i${ITERATION}n${NUM_AGENTS}r${ROUND}a${i}" >/dev/null 2>&1
            ) &
        done
